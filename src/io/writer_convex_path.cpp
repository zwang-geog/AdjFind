...
GDALDatasetH ConvexPathWriter::createRoadAccessPointDataset(const ConvexPathWriterConfig& config, OGRCoordinateTransformationH& coord_trans, std::string& output_file_path) {
    // Initialize coordinate transformation
    coord_trans = nullptr;
    ...
    // Modify output file path for road access point dataset
    output_file_path = config.output_file_path;
    fs::path path(output_file_path);
    std::string stem = path.stem().string();
    std::string extension = path.extension().string();
    output_file_path = (path.parent_path() / (stem + "_road_access_point" + extension)).string();
    ...
    // Use default layer name
    std::string layer_name = "road_access_points";
    OGRLayerH layer = GDALDatasetCreateLayer(dataset, layer_name.c_str(), layer_srs, wkbPoint, nullptr);
    if (!layer) {
        last_error_ = "Failed to create layer: " + layer_name;
        if (coord_trans) {
    ...
    // Create fields (only row_id and polygon_feature_id)
    OGRFieldDefnH row_id_field = OGR_Fld_Create("row_id", OFTInteger64);
    OGR_L_CreateField(layer, row_id_field, 1);
    OGR_Fld_Destroy(row_id_field);
...
bool ConvexPathWriter::writeRoadAccessPointFeatures(GDALDatasetH dataset, OGRLayerH layer,
                                                   OGRCoordinateTransformationH coord_trans,
                                                   const std::vector<graph::ConvexPathResult>& vertex_paths,
                                                   size_t polygon_feature_id,
                                                   size_t& feature_id) {
    // Write a feature for each path that has a valid geometry (extract last point)
    for (const auto& path_result : vertex_paths) {
        // Only write if path was found and has geometry
        if (!path_result.path_found || path_result.path_geometry.empty()) {
            continue;
        }
        
        // Extract last point from path geometry
        const auto& last_point = path_result.path_geometry.back();
        ...
        OGR_F_SetGeometry(feature, point_geom);
        OGR_G_DestroyGeometry(point_geom);
...
bool ConvexPathWriter::writeConvexPathResults(const ConvexPathWriterConfig& config, 
    ...
    // Create road access point dataset if requested (only when road data is used)
    GDALDatasetH road_access_point_dataset = nullptr;
    OGRCoordinateTransformationH road_access_point_coord_trans = nullptr;
    std::string road_access_point_output_file_path;
    OGRLayerH road_access_point_layer = nullptr;
    
    if (config.output_road_access_point && config.use_road_data) {
        road_access_point_dataset = createRoadAccessPointDataset(config, road_access_point_coord_trans, road_access_point_output_file_path);
        if (!road_access_point_dataset) {
            if (linestring_coord_trans) {
                OCTDestroyCoordinateTransformation(linestring_coord_trans);
            }
            if (point_coord_trans) {
        ...
        road_access_point_layer = GDALDatasetGetLayer(road_access_point_dataset, 0);
        if (!road_access_point_layer) {
            last_error_ = "Failed to get road access point layer from dataset";
            if (linestring_coord_trans) {
                OCTDestroyCoordinateTransformation(linestring_coord_trans);
            }
            if (point_coord_trans) {
            ...
            if (road_access_point_coord_trans) {
                OCTDestroyCoordinateTransformation(road_access_point_coord_trans);
            }
            GDALClose(linestring_dataset);
            GDALClose(point_dataset);
            GDALClose(road_access_point_dataset);
            return false;
        }
    }
    ...
    size_t road_access_point_feature_id = 0;
    ...
    for (const auto& polygon_result : polygon_results) {
        ...
        // Write road access point features (if requested and road data is used)
        if (config.output_road_access_point && config.use_road_data && road_access_point_layer) {
            if (!writeRoadAccessPointFeatures(road_access_point_dataset, road_access_point_layer, 
                                             road_access_point_coord_trans, 
                                             vertex_paths, polygon_feature_id, 
                                             road_access_point_feature_id)) {
                last_error_ = "Failed to write road access point features for polygon " + std::to_string(polygon_feature_id);
                break;
            }
        }
    ...
    if (last_error_.empty()) {
        ...
        std::cout << "  Path linestring dataset: " << linestring_output_file_path << std::endl;
        std::cout << "  Least-accessible-point dataset: " << point_output_file_path << std::endl;
        if (config.output_road_access_point && config.use_road_data) {
            std::cout << "  Road access point dataset: " << road_access_point_output_file_path << std::endl;
            std::cout << "  Total features written: " << feature_id << " path linestrings, " << point_feature_id << " least accessible points, " << road_access_point_feature_id << " road access points" << std::endl;
        } else {
            std::cout << "  Total features written: " << feature_id << " path linestrings, " << point_feature_id << " least accessible points" << std::endl;
        }
    }
